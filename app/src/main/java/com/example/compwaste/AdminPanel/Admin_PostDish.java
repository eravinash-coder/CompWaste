...
public class Admin_PostDish extends AppCompatActivity {
    ...
    private boolean isValid() {
        ...
        if (TextUtils.isEmpty(price)) {
            ...
            pri.setError("Reward is Required");
        } else {
            isValidPrice = true;
        }
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    ref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            WasteSupplyDetails info = new WasteSupplyDetails(dishes, quantity, price, description, String.valueOf(uri), RandomUId, ChefId);
                            firebaseDatabase.getInstance().getReference("WasteSupplyDetails").child(State).child(City).child(Sub).child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(RandomUId)
                                    .setValue(info).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    progressDialog.dismiss();
                                    Toast.makeText(Admin_PostDish.this, "Requirement posted successfully", Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
                    });
